...
func handleConfdbControlAction(c *Command, r *http.Request, user *auth.UserState) Response {
	...
	switch a.Action {
	case "delegate":
		err = cc.Delegate(a.OperatorID, a.Views, a.Authentications)
	case "undelegate":
		err = cc.Undelegate(a.OperatorID, a.Views, a.Authentications)
	default:
	...
	groups := cc.Groups()
	rawGroups := make([]interface{}, len(groups))
	for i, g := range groups {
		rawGroups[i] = map[string]interface{}{
			"operators":       interfaceSlice(g.Operators),
			"authentications": interfaceSlice(g.Authentications),
			"views":           interfaceSlice(g.Views),
		}
	}

	cc, err = devicestateSignConfdbControl(devMgr, rawGroups, cc.Revision()+1)
	if err != nil {
		return InternalError(err.Error())
	}
...
func getOrCreateConfdbControl(st *state.State, devMgr *devicestate.DeviceManager) (*asserts.ConfdbControl, error) {
	...
	if errors.Is(err, &asserts.NotFoundError{}) {
		return asserts.NewConfdbControl(serial.BrandID(), serial.Model(), serial.Serial()), nil
	}
	if err != nil {
		return nil, err
	}
