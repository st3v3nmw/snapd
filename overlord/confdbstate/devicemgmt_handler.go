...
func (h *confdbMessageHandler) Apply(st *state.State, msg *devicemgmtstate.RequestMessage) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("cannot decode confdb message body: %v", err)
	}
...
// ResultFromChange reads the completed confdb change and returns the response body.
func (h *confdbMessageHandler) ResultFromChange(chg *state.Change) (map[string]any, error) {
	...
	if hasErr {
		...
		if ok {
			return nil, &confdbHandlerError{body: errMap}
		}
...
// confdbHandlerError carries a structured error body from a completed change.
// The caller can use errors.As to extract the body for the response.
type confdbHandlerError struct {
	body map[string]any
}

func (e *confdbHandlerError) Error() string {
	if msg, ok := e.body["message"].(string); ok {
		return msg
	}
	return "confdb handler error"
}

// Body returns the structured error body for use in the response.
func (e *confdbHandlerError) Body() map[string]any {
	return e.body
}
